...
const CargoForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!destination || !name || !cargoType || !weight || !volume || phone.replace(/\D/g, '').length < 11) {
        ...
        <form className="w-full" onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 md:grid-cols-4 grid-rows-[78px 58px 58px 58px 58px 58px 58px] gap-x-[10px] md:gap-x-[20px] gap-y-[14px]">
            ...
            <div className="flex flex-col col-span-2 relative" ref={destinationRef}>
              ...
              <div 
                className={`bg-white text-black py-[8px] md:py-[20px] px-[16px] text-[12px] md:text-[16px] border-b border-gray-200 w-full cursor-pointer flex justify-between items-center transition-[border-radius] ${isDestinationOpen ? 'rounded-t-[10px] rounded-b-none' : 'rounded-[10px]'}`}
                onClick={() => setIsDestinationOpen(!isDestinationOpen)}
              >
                <span>{destination || 'Выберите город'}</span>
                <svg 
                  width="24" 
                  height="24" 
                  viewBox="0 0 24 24" 
              ...
              <div className={`absolute top-full left-0 right-0 z-10 bg-white text-black overflow-hidden transition-all min-w-full shadow-lg ${isDestinationOpen ? 'max-h-[300px] rounded-b-[10px]' : 'max-h-0'}`}>
                <div className="overflow-y-auto max-h-[300px]">
                  {cities.map((city) => (
                    <div
                      key={city.name}
                      className={`py-[10px] md:py-[15px] px-[16px] text-[12px] md:text-[16px] cursor-pointer hover:bg-gray-100 ${destination === city.name ? 'bg-gray-100 font-medium' : ''}`}
                      onClick={() => {
            ...
            <button 
              type="submit"
              className="flex justify-center items-center bg-[#344E74] hover:bg-[#506fa1] active:bg-white border-1 border-[#344E74] active:text-[#344E74] text-white md:font-bold text-[14px] md:text-[24px] xl:text-[36px] h-[37px] md:h-[64px] xl:h-[84px] col-span-2 md:col-span-4 rounded-[10px] transition-colors mt-[18px]"
            >
             {isSubmitting ? (
